...
class OrderController extends Controller
{
    ...
    use HasApiTokens, Notifiable,OrderTrait;
    ...
    public function addToCart(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'type' => 'sometimes|nullable|' . Rule::in([150, 200, 300, 500]),
        ]);
        if ($validator->fails()) {
            return $this->sendError(' ', $validator->errors()->first());
        }
        ...
        $findCart = Cart::where('customer_id', authCustomerApi()->id)->where('product_id', $request->product_id)->when($request->type, function ($q) use ($request) {
    ...
    public function createOrder(Request $request)
    {
        ...
        $order =  Order::create([
            "customer_id" => authCustomerApi()->id,
            // "address_id" => authCustomerApi()->address ? authCustomerApi()->address->id : '',
    ...
    public function CreateOrderDetails($order_id)
    {
        ...
        foreach ($carts as $item) {
            ...
            OrderDetail::create([
                ...
                'price_before_discount' => $item->type ? $item->product->Total * (int)($item->type) / 1000 : $item->product->Total,
    ...
    public function promoOrder($code)
    {
        if ($code) {
            ...
            if ($promocode) {
                //totalWithPromo
                $total = round($promocode->subtotal($this->sum_Incart()));
            } else {
                // $total = 0;
