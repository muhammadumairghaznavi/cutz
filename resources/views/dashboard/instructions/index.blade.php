    ...
    <section class="content">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title" style="margin-bottom: 15px">@lang('site.instructions')
                    <small>
                ...
                <form action="{{ route('dashboard.instructions.index') }}" method="get">
                    <div class="row">
                        ...
                        <div class="col-md-4 {{$section_add=='close'?'hidden':''}}">
                            @if (auth()->user()->hasPermission('create_instructions'))
                            <a href="{{ route('dashboard.instructions.create') }}" class="btn btn-primary"><i
            ...
            <div class="box-body">
                ...
                <table class="table table-hover" id="data_table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td> {{ $instruction->product->title ?? "" }}</td>
                            <td> {{ $instruction->title }}</td>
                            {{-- <td> {{ $instruction->description }}</td> --}}
                            <td><img src="{{ $instruction->image_path }}" style="width: 100px;" class="img-thumbnail" alt="">
                            </td>
                            <td>
                                @if (auth()->user()->hasPermission('update_instructions'))
                                    <a href="{{ route('dashboard.instructions.duplicate', $instruction->id) }}"
                                    class="btn btn-warning btn-sm {{$section_edit=='close'?'hidden':''}}"><i
                                        class="fa fa-copy"></i> @lang('site.duplicate')</a>

                                <a href="{{ route('dashboard.instructions.edit', $instruction->id) }}"
                                ...
                                <a href="#" class="btn btn-info btn-sm disabled"><i class="fa fa-edit"></i>
                                    @lang('site.edit')</a>
                                @endif
                                @if (auth()->user()->hasPermission('delete_instructions'))
                                <form action="{{ route('dashboard.instructions.destroy', $instruction->id) }}" method="post"
