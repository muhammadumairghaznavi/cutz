                      ...
                      <div class="form-group">
                          <label>@lang('site.sections')</label>
                            <select name="category_id" id="" class="form-control">
                                ...
                                <option  {{$item->id==$subCategory->category_id?'selected':''}} value="{{$item->id}}">{{$item->title}}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        @foreach (config('translatable.locales') as $locale)
                            <div class="form-group">
                                ...
                                <input type="text" name="{{ $locale }}[title]" class="form-control" value="{{ $subCategory->translate($locale)->title }}">
                            </div>
                        @endforeach
                        ...
                        <div class="form-group">
                            <img src="{{ $subCategory->image_path }}" style="width: 100px" class="img-thumbnail image-preview" alt="">
                        </div>
